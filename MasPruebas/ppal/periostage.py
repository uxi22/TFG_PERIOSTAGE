...
from PySide6.QtGui import (QRegularExpressionValidator, QImage, QPolygon, QBrush, QColor,
                           QPainter, QPen, QFontMetrics, QFont, QIcon, QAction, QShortcut, QKeySequence)
from PySide6.QtWidgets import (QApplication, QLabel, QDateEdit, QRadioButton, QMainWindow,
                               ...
                               QFileDialog, QCheckBox, QScrollArea, QToolBar, QMessageBox, QStyle)
...
class RecuadroPreguntaRadio(QFrame):
    def __init__(self, parent, coordenadas, pregunta, opciones, spacing=20, fs=18):
        ...
        self.setGeometry(coordenadas[0], coordenadas[1], coordenadas[2], coordenadas[3])
        self.setStyleSheet(
            "background-color: #DBDBDB; font-size: " + str(fs) + "px;")
...
class windowIni(QMainWindow):
    ...
    def showEvent(self, event):
        ...
        # Configurar el atajo después de que la ventana se muestre
        atajo = QShortcut(QKeySequence("Alt+Right"), self)
        atajo.activated.connect(lambda: siguientePantalla())

    def elementos(self):
        self.framePpal = QFrame(self.frameTodo)
        self.framePpal.setGeometry(0, 80, 920, self.height() - 80)
        self.framePpal.setStyleSheet("font-family: Alata; border-radius: 16px;")
...
class Datos:
    ...
    def extraerDatos(self):
        dir = os.path.join(basedir, "../datos" + datetime.datetime.now().strftime(
            "%y%m%d%H%M%S") + ".xlsx")
        ...
        if ruta != "":
            ...
            for i in range(len(dientes)):
                ...
                if i not in self.desactivadosInferior and i not in self.desactivadosSuperior and diente in self.puntosMuestreo:
                    ...
                    dfs.append(DataFrame(data=data, columns=[diente]))
                    data.clear()
                    ...
                    for j in range(1, 3):
                        ...
                        data = [1 if (x == True and isinstance(x, bool)) else 0 if isinstance(x, bool) else x for x in
                                data]
                        dfs.append(DataFrame(data=data, columns=[""]))
            ...
            # DATOS CALCULADOS
            datadf3 = []
            datadf3 += [window.clasificacion.text]
            # Cuantitativos:
            ...
            df3 = DataFrame(data=datadf3)
            df3.index = ["Diagnóstico", "Número total implantes", "Número dientes naturales", "PPD media", "CAL media",
...
class Clasificacion(QLabel):
    def __init__(self, parent):
        global colorClasificacion
        super().__init__(parent)
        self.setStyleSheet("font-weight: bold; font-size: 16px; margin: 5px;" "color: " + colorClasificacion + "; padding: 3px; border: 1px solid grey;")
        self.setText("")
...
class WindowDientes(QMainWindow):
    ...
    def showEvent(self, event):
        super().showEvent(event)
        # Configurar el atajo después de que la ventana se muestre
        atajo1 = QShortcut(QKeySequence("Alt+Right"), self)
        atajo1.activated.connect(lambda: siguientePantalla())
        atajo2 = QShortcut(QKeySequence("Alt+Left"), self)
        atajo2.activated.connect(lambda: anteriorPantalla())
        atajonew = QShortcut(QKeySequence("Ctrl+N"), self)
        atajonew.activated.connect(lambda: nuevoPeriodontograma())
    ...
    def actualizar_tam(self, event):
        ...
        self.scroll_area.setGeometry(0, 30, self.width(), self.height() - 30)
    ...
    def closeEvent(self, event):
        ...
        """reply = QMessageBox.warning(self, 'Cerrar aplicación', "¿Guardar los cambios antes de salir?",
                                     ...
                                     QMessageBox.Save)"""
        msg_box = QMessageBox(QMessageBox.Warning, 'Cerrar aplicación', "¿Guardar los cambios antes de salir?",
                              QMessageBox.Save | QMessageBox.Discard | QMessageBox.Cancel)

        msg_box.button(QMessageBox.Save).setText("Guardar y salir")
        msg_box.button(QMessageBox.Discard).setText("Salir sin guardar")
...
class WindowFinal(QMainWindow):
    def __init__(self):
        ...
        icono = QApplication.style().standardIcon(QStyle.SP_DialogSaveButton)
    ...
    def showEvent(self, event):
        super().showEvent(event)
        # Configurar el atajo después de que la ventana se muestre
        atajo = QShortcut(QKeySequence("Alt+Left"), self)
        atajo.activated.connect(lambda: anteriorPantalla())
        atajoguardar = QShortcut(QKeySequence("Ctrl+S"), self)
        atajoguardar.activated.connect(lambda: datos.extraerDatos())
        atajonew = QShortcut(QKeySequence("Ctrl+N"), self)
        atajonew.activated.connect(lambda: nuevoPeriodontograma())

    def actualizar_tam(self, event):
        self.frameTodo.setGeometry(0, 30, max(self.width(), 1030), 820)
